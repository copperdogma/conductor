...
description: Close out current Conductor work and any linked target-project worktrees in scope, optionally close a story if one is in scope, then run the full check-in and push-to-main flow
user-invocable: true
---
...
By default, "current Conductor work" includes both:

- the local Conductor diff
- any linked target-project repo worktrees or branches created or updated as
  part of the same supervisor request, story, alignment, or scout line

Do not require the user to restate "plus the remote repo changes" unless they
want to narrow the landing set.
...
- run `/mark-story-done`
- validate linked target-project worktrees or branches that are in scope
...
- fast-forward `main`
- push `main`
- repeat that landing flow in each linked target-project repo that is in scope
...
1. Resolve the intent surface:
   - read current git context for Conductor and any linked target-project
   ...
   - identify any target-project repos with dedicated worktrees or branches
     created or updated as part of that same supervisor line; treat them as in
     scope by default unless the user explicitly narrowed scope
   - if `inbox.md` changed during the work, treat that as normal user capture
     that should ride along with the landing by default, not as unrelated drift
...
2. Validate the work against that intent surface:
   ...
   - if linked target-project worktrees or branches are in scope, validate each
     of them against the same supervisor intent plus the repo-local diff and
...
3. If linked target-project worktrees or branches are in scope, land them first:
   - use each target repo's own close-out and landing skill or equivalent
     repo-native workflow
   - keep each repo isolated in its own worktree or branch; do not collapse
     target-project edits back into the primary checkout
   - stop if any target repo has a major gap, fails validation, or cannot land
     cleanly onto its own `main`

4. If a completed story is genuinely in scope, run `/mark-story-done` first:
...
- linked target-project worktrees or branches in scope that are still unlanded,
  unvalidated, or ambiguously related to the same supervisor intent
...
- unrelated, risky, or suspicious git changes in the landing set, except for
  `inbox.md` edits that reflect normal user capture
...
- Never silently drop linked target-project changes from scope when they were
  prepared as part of the same supervisor work; require explicit user narrowing
  to exclude them
...
- Never treat `inbox.md` user-capture edits as a close-out blocker; they should
  be included in the landing set by default unless the user says otherwise
