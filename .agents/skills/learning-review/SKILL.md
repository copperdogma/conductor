...
evidence that Conductor's workflow may need to learn from what just happened.

This is a detector. It does not edit live skills, `AGENTS.md`, memory,
methodology docs, candidate files, or target-project repos. When a candidate is
warranted, hand the finding to `/learning-candidate`.
...
- generic process advice with no Conductor-specific target
...
- the active story, scout, alignment note, or user request
...
Target surface: <skill, runbook, memory, ADR, story loop, alignment/scout lane, or none yet>
...
- Do not change target-project repos from this skill.
- Prefer one high-quality candidate over several speculative ones.
- If the candidate target is cross-project methodology, keep it
  recommendation-first until target projects have their own stories or explicit
  rollout approval.
