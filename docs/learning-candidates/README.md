...
This folder holds reviewed workflow-learning candidates for Conductor.
...
- Live skills, `AGENTS.md`, methodology docs, memory, and target projects
  change only through explicit promotion.
...
- created date
- target surface: the exact skill, doc, memory path, story workflow, or
  target-project workflow that would change if promoted
- trigger class
...
- promotion action: the exact file, skill, memory path, story, alignment note,
  or target-project workflow changed after acceptance
- validation plan
- decision history
